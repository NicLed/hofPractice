...
var onlyOneFruit = function(fruits, targetFruit) {
  var favoriteFruit = _.filter(fruits, function(fruit) {
...
var cookiesOnly = function(desserts) {
  var dessertType = _.filter(desserts, function(dessert) {
    if (dessert['cookie']) {
      return dessert;
    }
  });
...
var sumTotal = function(products) {
  var totalPrice = _.reduce(groceries, function(memo, product) {
    var stringPrice = product.price.substring(1);
    var numberPrice = Number(stringPrice);
    return memo + numberPrice;
  }, 0);
  return totalPrice;
};
...
var dessertCategories = function(desserts) {
  ...
  _.reduce(desserts, function(memo, dessert) {
    if (dessertTypes[dessert.type] === undefined) {
      return dessertTypes[dessert.type] = 1;
    } else {
      return dessertTypes[dessert.type]++;
    }
  }, 0);
  return dessertTypes;
};
